<template>
  <div>
    <BasicModal v-bind="$attrs" @register="register" title="导入EXCEL" :width="600"
                @cancel="handleClose" :confirmLoading="uploading" destroyOnClose>
      <!--是否校验-->
<!--      <div style="margin: 0 5px 5px" v-if="online">-->
<!--        <span-->
<!--          style="display: inline-block; height: 32px; line-height: 32px; vertical-align: middle">是否开启校验:</span>-->
<!--        <span style="margin-left: 6px">-->
<!--          <a-switch :checked="validateStatus == 1" @change="handleChangeValidateStatus"-->
...
<!--      </div>-->
      ...
      <template #footer>
        ...
        <a-button type="primary" @click="handleImport" :disabled="uploadDisabled"
                  :loading="uploading">{{
            uploading ? '上传中...' : '开始上传'
          }}
        </a-button>
...
export default defineComponent({
  ...
  setup(props, {emit, refs}) {
    ...
    function handleImport() {
      ...
      console.log(props.url, inputValue.value)
      defHttp.post({
        url: props.url,
        params: formData,
        headers
